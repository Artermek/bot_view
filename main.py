...
def mock_request_openai(system: str, user: str, model: str = 'gpt-4.1-2025-04-14', temp: Optional[float] = None):
    ...
    logging.info(f"Mock OpenAI called with user prompt: {user[:100]}...")

    if any(keyword in user.lower() for keyword in ["психологический отчёт", "анализ ребёнка"]):
        return f"""
**Психологический отчёт о ребёнке 8 лет**
## 📚 Краткая сводка

* **Имя ребёнка:** ...
* **Главное качество (рисунок "Дом")**: Чувство защищённости.
* **Основная черта (рисунок "Животное")**: Воображение.
* **Самооценка (автопортрет)**: Умеренная самооценка.
...
| ------- | ------------------- | --------------------- |
| Дом     | Уютный, с окнами    | Потребность в безопасности |
| Дерево  | Пышная крона        | Жизненная энергия |
| Человек | Среднего размера    | Умеренная уверенность |

**Общий вывод:** Ребёнок демонстрирует стабильность и умеренную уверенность.
...
* **Выбор животного:** Лев
* **Акценты в рисунке:** Большие глаза
* **Позы и выражение:** Спокойная поза

**Вывод:** Хорошо развито воображение.

---

### 3. Автопортрет: особенности самовосприятия

* **Размер фигуры:** Средний
* **Выражение лица:** Нейтральное
* **Дополнительные детали:** Простой фон

**Вывод:** Умеренная самооценка, поддержка желательна.
...
| Шкала                      | Баллы (из 25) |
| -------------------------- | ------------- |
| Эмоциональная устойчивость | 15            |
| Социальная адаптация       | 17            |
| Саморегуляция              | 14            |
| Коммуникативность          | 16            |
| Самооценка                 | 13            |
...
* Хвалите ребёнка за усилия.
* Создавайте спокойную атмосферу.
* Поощряйте творчество.

---

*Отчёт составлен на основе тестовых данных.*
"""
    else:
        logging.warning(f"Mock OpenAI не распознал запрос: {user[:100]}...")
...
async def get_report(task_id: str):
    ...
    task_str = redis_client.get(task_id)
    task = json.loads(task_str)
    logging.info(f"Task ID: {task_id} | Photo Status: {task['photo_status']} | Survey Status: {task['survey_status']}")
    if task['photo_status'] == 'done' and task['survey_status'] == 'done':
        ...
        Твоя цель сделать психологический отчёт , анализ ребёнка 
...
Нужно сделать психологический отчёт , анализ ребёнка 
...
*Отчёт составлен на основе проектных методик и наблюдений. Является ориентиром для мягкой поддержки ребёнка в развитии.*
        ...
        try:
            ...
            logging.info(f"Task ID: {task_id} | PDF создан: {pdf_filename}")
            return FileResponse(
                pdf_filename, 
