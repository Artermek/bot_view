...
def mock_request_openai(system: str, user: str, model: str = 'gpt-4.1-2025-04-14', temp: Optional[float] = None):
    if "психологический отчёт" in user.lower() or "анализ ребёнка" in user.lower():
        return """
**Психологический отчёт о ребёнке**
## Краткая сводка
- **Имя ребёнка:** Тестовый ребёнок
- **Главное качество:** Чувство защищённости
- **Самооценка:** Умеренная
...
"""
        ...
        return "Это предопределённый анализ опросника."
    else:
