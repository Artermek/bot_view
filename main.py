...
async def process_survey(task_id: str, survey: SurveyData):
    ...
    system_prompt = """Вы — опытный психолог, анализирующий ответы родителей на открытые вопросы анкеты о развитии ребенка, а так же интерпретирующий цифровые результаты
    закрытого теста для ребёнка (4 раздела по 10 вопросов, шкала 1–5)
    
    ⚙️  Правила классификации
    1. Для каждого раздела (10–50 баллов):
    • 10–19 — норма / благополучие  
    • 20–29 — лёгкие трудности  
    • 30–39 — умеренные трудности  
    • 40–50 — выраженные трудности  

    2. Для общей суммы G (40–200):
    • 40–79  — зона благополучия  
    • 80–119 — зона первых сигналов  
    • 120–159 — зона выраженных трудностей  
    • 160–200 — зона высокой тревоги  

    3. Расшифровка уровней:
    • Благополучие — развитие в пределах нормы, трудности эпизодичны.  
    • Первые сигналы — устойчивые, но умеренные сложности; при стрессе могут усиливаться.  
    • Выраженные трудности — проблемы заметно мешают учёбе и общению.  
    • Высокая тревога — серьёзный дистресс, нужен междисциплинарный подход.  

    📝  Требования к ответу  
    • Сначала перечислите четыре строки вида  
    «Эмоциональная сфера — 34 балла, уровень: умеренные трудности».  
    • Затем строку «Общая сумма: … | Уровень: …»  
    с кратким пояснением (2–3 предложения).  
    • После этого дайте рекомендации (3–5 пунктов, каждый с «– »).  
    • Выводите только текст, без JSON и Markdown.
    
    """
    user_prompt = f"""
    Результаты закрытого теста:

    1. Эмоциональная сфера: {scores['section_1']}
    2. Социальное взаимодействие: {scores['section_2']}
    3. Саморегуляция и поведение: {scores['section_3']}
    4. Самооценка и уверенность: {scores['section_4']}
    Общая сумма баллов: {scores['total']}

    Проанализируйте результаты и дайте вывод согласно инструкциям system prompt
...
async def get_report(task_id: str):
    ...
    if task["photo_status"] == "done" and task["survey_status"] == "done":
        ...
        Ты — опытный психолог, анализирующий результаты анализа фотографий и психологического отчёта о ребёнке.
            ...
            "questionnaire": {
                "scales": [
                { "name": "Эмоциональная устойчивость", "score": 22, "transcript": "лёгкие трудности"},
                { "name": "Социальная адаптация",       "score": 32, "transcript": "умеренные трудности"},
                { "name": "Саморегуляция",              "score": 12, "transcript": "норма / благополучие"},
                { "name": "Самооценка",                 "score": 42, "transcript": "выраженные трудности."},
                { "name": "Общее",                      "score": 108, "transcript": "зона первых сигналов."}
                ]
            },
            
             "transcript_questionnaire": {
                "scales": [
                    В целом у ребёнка имеются устойчивые, но умеренные проблемы, особенно в социальном взаимодействии и самооценке, что может усугубляться при стрессах.
                    
                    Анализ открытых данных показывает, что ребенок гиперактивен, что может осложнять его поведение и коммуникацию. Таланты в танцах и пении 
                    свидетельствуют о развитых творческих способностях. Отмечается неблагоприятное отношение к общению, что согласуется с умеренными
                    трудностями в социальном взаимодействии. Ранее специалисты не привлекались, что важно учесть.
                ]
            },
